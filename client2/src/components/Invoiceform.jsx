...
const Invoiceform = () => {
  ...
  // Fetch products for dropdown with stock information
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('/products');
  ...
  const handleProductChange = (index, productId) => {
    ...
    if (selectedProd) {
      // Set the selected product for stock display
      setSelectedProduct({...selectedProd, index});
      ...
      newItems[index] = {
        ...
        gstpercent: selectedProd.gstpercent || 0,
  ...
  const handleItemChange = (index, field, value) => {
    ...
    // For quantity field, check if it exceeds available stock
    if (field === 'quantity') {
      const currentProductId = newItems[index].productid;
      if (currentProductId) {
        const productStock = products.find(p => p._id === currentProductId)?.availablestock || 0;
        const quantityValue = parseFloat(value) || 0;
        
        // Warn if quantity exceeds available stock
        if (quantityValue > productStock) {
          alert(`Warning: Quantity exceeds available stock (${productStock} ${newItems[index].unit || 'KG'})`);
        }
      }
    }
  ...
  const addRow = () => {
    ...
    // Reset the selected product when adding a new row
  ...
  const removeRow = (index) => {
    ...
    // If removed row was the selected product, reset selectedProduct
  ...
  const handleSubmit = async () => {
    // Validate if any item exceeds available stock
    const invalidItems = items.filter(item => {
      if (!item.productid) return false;
      const product = products.find(p => p._id === item.productid);
      return product && parseFloat(item.quantity) > product.availablestock;
    });

    if (invalidItems.length > 0) {
      const confirmation = window.confirm("Some items exceed available stock. Do you still want to proceed?");
  ...
  // Get the currently selected product for a specific row
  const getSelectedProductForRow = (index) => {
            ...
            <div className="overflow-x-auto rounded-lg shadow">
              <table className="w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
                    <th className="px-4 py-3 text-left text-sm font-medium">Product</th>
                    <th className="px-4 py-3 text-left text-sm font-medium">Available Stock</th>
                    <th className="px-4 py-3 text-left text-sm font-medium">Quantity</th>
                    <th className="px-4 py-3 text-left text-sm font-medium">Price/Unit</th>
                    <th className="px-4 py-3 text-left text-sm font-medium">Discount (%)</th>
                    <th className="px-4 py-3 text-left text-sm font-medium">GST (%)</th>
                    <th className="px-4 py-3 text-left text-sm font-medium">Net Amount</th>
                    <th className="px-4 py-3 text-left text-sm font-medium">Action</th>
                ...
                <tbody>
                  {items.map((item, index) => {
                    const currentProduct = getSelectedProductForRow(index);
                    return (
                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                      <td className="px-4 py-3 border-t border-gray-200">
                        <select
                        ...
                        >
                          ...
                          {Array.isArray(products) && products.map(product => (
                            ...
                            >
                              {product.name} (₹{product.price}/{product.unit || 'KG'})
                            </option>
                          ))}
                        </select>
                      </td>
                      <td className="px-4 py-3 border-t border-gray-200">
                        {currentProduct && (
                          <StockInfoDisplay product={currentProduct} />
                        )}
                      </td>
                      <td className="px-4 py-3 border-t border-gray-200">
                        <input
                          ...
                          onChange={(e) => handleItemChange(index, 'quantity', e.target.value)}
                          className={`border ${
                            currentProduct && parseFloat(item.quantity) > currentProduct.availablestock
                              ? 'border-red-500 bg-red-50'
                              : 'border-gray-300'
                          } p-2 rounded-lg w-full focus:ring-2 focus:ring-blue-400 focus:border-transparent`}
                        />
                        {currentProduct && parseFloat(item.quantity) > currentProduct.availablestock && (
